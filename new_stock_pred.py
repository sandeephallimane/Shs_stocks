...
def send_email(subject, html_content, receiver_emails):
    return requests.post(
        f"https://api.mailgun.net/v3/{md}/messages",
        auth=("api", ma),
        data={
            "from": "Sandeep@test.com",
            "to": receiver_emails,
            "subject": subject,
            "html": html_content
        }
...
if len(forecasts)> 0:
  response =send_email(ms, email_body, "hssandp@gmail.com")
  print("Email sent! Status code:", response.status_code, response.text)

else:
  response = send_email(ms, e_body1, "hssandp@gmail.com") 
  print("Email sent! Status code:", response.status_code, response.text)
